...
def get_arguments():
    ...
    parser.add_argument('--model_name', type=str, default='vgg11', help='model name', required=True)
    parser.add_argument('--data_dir', type=str, help='dataset path', required=True)
    parser.add_argument('--output_dir', type=str, help='output path', required=True)
...
def valid(model, device, valid_dataloader, criterion):
    ...
    correct = 0
    with torch.no_grad():
        for batch_idx, (data, target) in tqdm(enumerate(valid_dataloader)):
            data, target = data.to(device), target.to(device)
...
def main():
    ...
    # train_dataset = torchvision.datasets.ImageNet(root=args.data_dir, split='train', transform=transform_train)
    # valid_dataset = torchvision.datasets.ImageNet(root=args.data_dir, split='val', transform=transform_valid)
    train_dataset = torchvision.datasets.STL10(root=args.data_dir, download=True, split='train', transform=transform_train)
    valid_dataset = torchvision.datasets.STL10(root=args.data_dir, download=True, split='test', transform=transform_valid)
    ...
    # set model
    model = models.__dict__[args.model_name](in_channels=3, num_classes=10).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if valid_acc > best_acc:
            best_acc = valid_acc
            torch.save(model.state_dict(), os.path.join(args.output_dir, args.model_name + '_best.pth'))
